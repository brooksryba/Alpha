...
public class StateStartTurn : StateMachineBehaviour
{
    ...
    override public void OnStateEnter(Animator animator, AnimatorStateInfo stateInfo, int layerIndex)
    {
        ...
        for(int i = 1; i <= 2*_manager.condition.characterTurnOrder.Count; i++){
            _manager.turnIndex = (_manager.turnIndex + 1) % (_manager.condition.characterTurnOrder.Count);
            if(_manager.turnIndex == 0){
                _manager.condition.characterTurnOrder.Sort(delegate(Character a, Character b){return (b.characterClass.speed).CompareTo(a.characterClass.speed);}); // highest speed first (a comp to b is lowest)
                _manager.overallTurnNumber += 1;

            }
            if(!_manager.condition.deadPlayerList.Contains(_manager.condition.characterTurnOrder[_manager.turnIndex].title)){
                break;
            }

        }
        
        Character nextUp = _manager.condition.characterTurnOrder[_manager.turnIndex];
